...
Collection.prototype.update = function update(criteria, values, cb) {
  var self = this, upsert = false, upsertOnly = false,
      query;
  ...
  if (typeof criteria === 'object') {
    ...
    // added an upsert flag if we want to upsert
    upsert = criteria.upsert;
    delete criteria.upsert;
  }
  ...
  collection.find(query.criteria.where, {_id: 1}).toArray(function(err, records) {
    ...
    // Update the records
    collection.update(query.criteria.where, { '$set': values }, { multi: true , upsert}, function(err, result) {
      if(err) return cb(err);

      if (upsertOnly && result && result.result && result.result.upserted) {
        updatedRecords = result.result.upserted.map(doc => doc._id);
      }
